...
class OctopusAnimator {
    constructor(canvasOrId, opts = {}) {
        ...
        this.state = {
            ...
            _lastSyncDebugTs: 0,
            actionJawScale: 1,
            eyeLookX: 0,         // pupil offset X
            eyeLookY: 0,         // pupil offset Y
            fx: []
        };
    ...
    handleCommand(cmd) {
        ...
        if (cmd && cmd.cmd === "gaze") {
            const g = cmd.gaze || {};
            // Horizontal fix: remove inversion. Boost range.
            const gx = (typeof g.x === "number") ? g.x * 2.8 : 0;
            const gy = (typeof g.y === "number") ? g.y * 2.5 : 0;

            // Apply smoothing/clamping
            this.state.eyeLookX = Math.max(-1, Math.min(1, gx));
            this.state.eyeLookY = Math.max(-1, Math.min(1, gy));

            if (window.__gazeDebug) {
                console.log(`👁️ Gaze UI: x=${this.state.eyeLookX.toFixed(2)} y=${this.state.eyeLookY.toFixed(2)} blink=${g.blink}`);
            }
    ...
    render(timestamp) {
        ...
        let targetMouthValue = this.state.targetMouth;
        let targetJawValue = this.state.targetJaw;

        if (this.state.speaking) {
            // Speech is active. Use viseme + amplitude.
            // Scale jaw by action multiplier (e.g. 1.25 for laughing) to enhance emotion.
            targetJawValue *= jawScale;
            // If action is active, optionally keep a bit of smile/bias if it's positive.
            if (actionActive && mouthBias > 0.1) {
                targetMouthValue = Math.max(targetMouthValue, mouthBias * 0.7);
            }
        } else if (actionActive) {
            // Not speaking, but action is active (e.g. smiling for photo)
            targetMouthValue = mouthBias;
            targetJawValue = Math.max(0, mouthBias);
        } else {
            // Idle or neutral
            targetMouthValue = this.state.targetMouth; // Usually mouthFloor
            targetJawValue = 0;
        }

        const targetMouth = Math.max(0, Math.min(1, targetMouthValue));
        ...
        const targetJaw = Math.max(0, targetJawValue);
    ...
    drawEyes(cx, cy) {
        ...
        const drawEye = (x, y) => {
            ...
            // pupil - moves based on gaze or photobooth action
            const lookX = this.state.eyeLookX * 4; // Max 4px horizontal shift
            const lookY = (this.state.eyeLookY * 3) + (eyeLookDown * 4); // Combine gaze + action down-look
