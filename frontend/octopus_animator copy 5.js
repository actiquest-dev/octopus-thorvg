...
class OctopusAnimator {
     ...
     */
    handleCommand(cmd) {
        console.log(`📥 handleCommand: cmd=${cmd?.cmd || 'null'}, hasVisemes=${Array.isArray(cmd?.visemes)}`);
        ...
        if (cmd && cmd.cmd === "audio_sync") {
            ...
            // 🔥 Использовать audioContext.currentTime для точной синхронизации с аудио
            let now = performance.now();
            if (window.__audioPlayer && window.__audioPlayer.audioContext) {
                const audioCtx = window.__audioPlayer.audioContext;
                now = audioCtx.currentTime * 1000;  // Convert to milliseconds
            }

            let startTs = now;
            if (typeof cmd.start_s === "number" && typeof window.getAudioPlayheadS === "function") {
                const playhead = window.getAudioPlayheadS();
                // ИСПРАВЛЕНИЕ: Не сбрасывать responseStartPlayheadS каждый раз
                if (this.state.responseStartPlayheadS == null || cmd.start_s <= 0.001) {
