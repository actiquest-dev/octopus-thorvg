...
class FaceIDDetector {
  constructor() {
    ...
    this.faceDetectionModel = null; // TFLite model
  ...
  async processFrame(base64Image, width, height) {
    ...
    try {
      ...
      const imageData = await this._base64ToImageData(base64Image, width, height);

      // Detect faces using MediaPipe
      const results = await this.faceDetectionModel.send({ image: imageData });
   ...
   */
  async _base64ToImageData(base64, width, height) {
...
class SplitVideoStreamer {
  ...
  async _processFaceID(base64Image, width, height) {
    try {
      ...
      // Лица найдены - отправить на backend для идентификации
      for (const face of result.faces) {
        this._identifyFace(base64Image, face);
      }
  ...
  /**
   * Identify face by sending to backend API
   */
  async _identifyFace(base64Image, face) {
    try {
      ...
      // Вызвать callback с результатом
      if (typeof this.onFaceIDResult === "function") {
        this.onFaceIDResult({
          type: result.matched ? "face_identified" : "face_unknown",
      ...
      console.log(
        result.matched
          ? `✅ Identified: ${result.user_name} (confidence: ${result.confidence})`
          : "❌ Face not recognized"
      );
