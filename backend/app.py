...
# Face ID imports
try:
    from face_db_postgres import get_face_database, AsyncSessionLocal
    ...
    print(f"⚠️ Face ID features disabled: {e}")
    FACE_ID_AVAILABLE = False

print(f"ℹ️ FACE_ID_AVAILABLE = {FACE_ID_AVAILABLE}")
...
async def websocket_handler(client_ws):
    ...
    async def connect_google(service_url: str):
        ...
        token = creds.token
        print(f"📡 Connecting to: {service_url}", flush=True)
        print(f"🔑 Token: {token[:30]}... creds_path={creds_path}", flush=True)
        ...
        print("✅ Google connected, waiting for frontend setup message")
    ...
    async def browser_to_google():
        ...
        async for msg in client_ws:
            # DEBUG: Log ALL messages from browser
            msg_preview = msg[:100] if isinstance(msg, str) else str(msg)[:100]
            print(f"📨 RAW message from browser: {msg_preview}", flush=True)
    ...
    async def google_to_browser():
        ...
        _user_transcription_buffer = ""  # Buffer for STT fragments
        ...
        try:
            async for message in google_ws:
                ...
                if server_content.get("inputTranscription"):
                    ...
                    print("📝 inputTranscription chunk:", input_trans, flush=True)
                    ...
                    if FACE_ID_AVAILABLE and isinstance(input_trans, dict):
                        ...
                        _user_transcription_buffer += user_text

                        if is_finished:
                            full_text = _user_transcription_buffer.strip()
                            _user_transcription_buffer = "" 
                            
                            if full_text:
                                print(f"📝 Full buffered transcription: '{full_text}'", flush=True)
                                try:
                                    dialog_mgr = await get_dialog_manager()
                                    print(f"🔍 DEBUG: is_registering={dialog_mgr.is_registering}, current_user={dialog_mgr.current_user_id}", flush=True)
                                    result = await dialog_mgr.process_user_speech(full_text)
...
async def face_identify_handler(request):
    ...
    """
    print(f"🔴 /api/face/identify called, FACE_ID_AVAILABLE={FACE_ID_AVAILABLE}", flush=True)

    if not FACE_ID_AVAILABLE:
        print("⚠️ /api/face/identify called but FACE_ID_AVAILABLE is False", flush=True)
        return web.json_response({
            "error": "Face ID not available",
            "matched": False
        }, status=503)

    # print("🔍 /api/face/identify request received", flush=True) # Uncomment for verbose debug

    try:
        data = await request.json()
        image_b64 = data.get("image_b64", "")
        threshold = data.get("threshold", 0.6)
        print(f"🔴 Parsed: image_b64_len={len(image_b64)}, threshold={threshold}", flush=True)

        if not image_b64:
            print(f"🔴 Error: no image_b64", flush=True)
            return web.json_response({
        ...
        try:
            ...
            print(f"🔴 Decoded image bytes: {len(image_bytes)}", flush=True)
        except Exception as e:
        ...
        print(f"🔴 Calling detect_and_process_faces...", flush=True)
        try:
            from face_detector import detect_and_process_faces
            faces = await detect_and_process_faces(image_b64)
            print(f"🔴 detect_and_process_faces returned {len(faces)} faces with embeddings", flush=True)
        except Exception as e:
            print(f"⚠️ Face detection error: {e}", flush=True)
            import traceback
            traceback.print_exc()
            return web.json_response({
                "matched": False,
        ...
        if not faces:
            return web.json_response({"users": [], "timestamp": time.time()})
        ...
        try:
            ...
            for face in faces:
                ...
                if user:
                    ...
                    user_name = user["name"]
                    # Skip duplicates (same person detected twice)
                    if any(u["user_id"] == user_id for u in recognized):
                        continue
                    try:
                    ...
                    recognized.append({
                        "user_id": user_id,
                        "user_name": user_name,
                        "confidence": user.get("confidence", 0.9),
                        "user_history": {
                            "interaction_count": user_history.get("interaction_count", 0) if user_history else 0,
                            "average_emotion": user_history.get("average_emotion", "neutral") if user_history else "neutral",
                            "last_seen": user_history.get("last_seen") if user_history else None,
                        }
                    })
                    print(f"✅ Recognized: {user_name} (conf={user.get('confidence', 0):.2f})", flush=True)
                else:
                    has_unknown = True

            if has_unknown and not recognized:
                # Only trigger registration if NO known faces at all
                try:
                    dialog_mgr = await get_dialog_manager()
            ...
            return web.json_response({
                "users": recognized,
                "matched": len(recognized) > 0,
                "timestamp": time.time()
            })
        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"⚠️ Face DB error: {e}", flush=True)
            return web.json_response({"matched": False, "timestamp": time.time()})
...
async def admin_list_users_handler(request):
    """
    ...
    if not FACE_ID_AVAILABLE:
        return web.json_response(
            {"success": False, "error": "Face ID not available"},
            status=503
        )
    ...
    try:
        ...
        # Get all users with face count
        async with AsyncSessionLocal() as session:
            # Query to get users with their face profile count
            result = await session.execute(text("""
                SELECT u.id, u.name, u.created_at, COUNT(fp.id) as face_count
                FROM users u
            ...
            users = []
            for row in rows:
                user_id, name, created_at, face_count = row
                users.append({
                    "user_id": str(user_id),
                    "name": name,
        ...
        stats = {
            ...
            "total_profiles": stats_data.get("total_face_profiles", 0)
        }
...
async def config_handler(request):
    """Return client config (project_id for Vertex AI)"""
    project_id = os.environ.get("GEMINI_PROJECT_ID", "octopus-489714")
    print(f"📋 Config endpoint: project_id={project_id}", flush=True)
    return web.json_response({
